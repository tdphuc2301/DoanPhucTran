...
@section('content')
    <div class="row" id="object-category">
        ...
        @include('Admin.Rom.create-model')
    </div>
@endsection
@section('script')
    <script>
        var apiGetList = '{{ route('admin.rom.get_list') }}';
        var apiCreate = '{{ route('admin.rom.create') }}';
        var apiGetItem = '{{ route('admin.rom.get_category') }}';
        var apiChangeStatus = '{{ route('admin.rom.change_status') }}';
